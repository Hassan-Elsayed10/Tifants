...
export default function Restroom() {
    ...
    const [isFocusedRemind, setIsFocusedRemind] = useState(false);
    const [isFocusedDiaper, setIsFocusedDiaper] = useState(false);
    ...
    const [selectedDiaper, setSelectedDiaper] = useState(null);
    const [isListDiaperVisible, setListDiaperVisible] = useState(false);
    ...
    const Diaper = ['diaper1','diaper2']
    const data = ['Pizza', 'Snack', 'milk'];
    
    const handleToggle = () => {
        setIsFocused(!isFocused);
    };
    const handleToggleDiaper = () => {
        setIsFocusedDiaper(!isFocusedDiaper);
    };
    const handleToggleRemind = () => {
        setIsFocusedRemind(!isFocusedRemind);
    };
    ...
    const handlePressDiaper = () => {
        setListDiaperVisible(!isListDiaperVisible);
    };
    ...
    const handleDiaperSelect = (item) => {
        setSelectedDiaper(item);
        setListDiaperVisible(false);
    };
    ...
    const renderListDiaper = ({ item }) => {
        return (
            <TouchableOpacity style={styles.list} onPress={() => handleDiaperSelect(item)}>
                <Text style={{ color: '#000000', fontSize: 15, fontWeight: '400' }}>{item}</Text>
            </TouchableOpacity>
        );
    };
    ...
    return (
        <ScrollView style={styles.container}>
            <View style={styles.row} >
                ...
                <Text style={styles.text}>Restroom</Text>
            </View>

            <TouchableOpacity onPress={()=>navigation.navigate('SelectionRestroom')}>
                ...
                {isListVisible ? (
                    ...
                    <TouchableOpacity style={styles.selectContainer} onPress={handlePress}>
                        {selectedItem ?
                            ...
                            :
                            <Text style={{ color: '#565656', fontSize: 16, fontWeight: '500', flex: 1 }}>Restroom Type</Text>
                        }
            ...
            <View style={{ margin: 5, padding: 5 }}>
                {isListDiaperVisible ? (
                    <FlatList
                        data={Diaper}
                        renderItem={renderListDiaper}
                        keyExtractor={(item) => item}
                    />
                ) :
                    <TouchableOpacity style={styles.selectContainer} onPress={handlePressDiaper}>
                        {selectedDiaper ?
                            <Text style={{ color: '#565656', fontSize: 16, fontWeight: '500', flex: 1 }}>{selectedDiaper}</Text>
                            :
                            <Text style={{ color: '#565656', fontSize: 16, fontWeight: '500', flex: 1 }}>Diaper Type</Text>
                        }
                        <Image source={require('../../../Assets/Frame(4).png')} />
                    </TouchableOpacity>
                }
                ...
                {isListQuantityVisible ? (
                    ...
                    <TouchableOpacity style={styles.selectContainer} onPress={handleQuantityPress}>
                        {selectedQuantity ?
                            ...
                            :
                            <Text style={{ color: '#565656', fontSize: 16, fontWeight: '500', flex: 1 }}>Diaper Size</Text>
                        }
                        <Image source={require('../../../Assets/Frame(4).png')} />
                    </TouchableOpacity>
                }
            ...
            <View style={styles.togglContainer}>
                <Text style={styles.toggelText}>Diaper cream applied</Text>
                <TouchableOpacity onPress={handleToggleDiaper} >
                    <Icon
                        name={isFocusedDiaper ? 'toggle-on' : 'toggle-off'}
                        size={30}
                        color={isFocusedDiaper ? '#475AD7' : '#D9D9D9'}
                    />
                </TouchableOpacity>
            </View>
            ...
            <View style={styles.togglContainer}>
                <Text style={styles.toggelText}>Remind me in 15 minutes</Text>
                <TouchableOpacity onPress={handleToggleRemind} >
                    <Icon
                        name={isFocusedRemind ? 'toggle-on' : 'toggle-off'}
                        size={30}
                        color={isFocusedRemind ? '#475AD7' : '#D9D9D9'}
                    />
                </TouchableOpacity>
            </View>
